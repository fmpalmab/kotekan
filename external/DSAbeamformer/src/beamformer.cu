...
int run_beamformer(int argc, char *argv[]){
	...
	*********************************************************************************/
	...
	#if BURNIN && !DEBUG
		std::cout << "Burning IN disabled" << std::endl;
	#endif
	...
	while (!obs_state.check_observations_complete()){
		...
		**************************************************/
		...
		if (obs_state.check_ready_for_transfer()){
			#ifndef DEBUG
				#if VERBOSE
					std::cout << "READING FROM KOTEKAN" << std::endl;
				#endif
				
				// TODO: Read from Kotekan buffers.
				// block = kotekan_buffer.read();

				// if (!kotekan_transfers_complete){
					
					/* Copy Block */
					// gpuErrchk(cudaMemcpyAsync(&d_data[N_BYTES_PRE_EXPANSION_PER_BLOCK * obs_state.get_next_gpu_transfer_block()],
					// 							block,
		...
		**************************************************/
		...
		std::cout << "Approximate datarate: " << N_BYTES_PRE_EXPANSION_PER_GEMM*input_data_generator.get_n_pt_sources()/observation_time_ms/1e6 << "GB/s" << std::endl;
			...
			std::cout << "Approximate datarate: TBD GB/s" << std::endl;
